...
const get_current_stock_report = async (request, res) => {
      try {
            let data = await get_current_stock_data(request);
            console.log(data.length);
            if (!data || data.length === 0) {
                  log.info("No Current stock report Found");
                  return res.status(404).json({
                        code: 404,
                        message: "No Current stock report Found",
                        data: null,
                  });
            }
            const buffer = await generate_current_stock_xlsx(data);
            const timestamp = Date.now();
            const file_name = `current_stock_report_${timestamp}.xlsx`;

            log.info(`Download current stock report - [${file_name}]`);
...
const get_current_stock_data = async (request) => {
      let data = null;
      try {
            ...
            log.error(`An exception occurred while getting current stock report information: ${err?.message}`);
            throw err;
      }
      return data;
}

const generate_current_stock_xlsx = async (data) => {
      ...
      addReportHeader(sheet, "Current Stock Report", titles.length);

      // Add column titles (bold)
      const headerRowIndex = 3; // Titles go on row 5
...
module.exports = get_current_stock_report;